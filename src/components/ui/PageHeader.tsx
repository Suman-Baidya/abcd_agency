  ...
  return (
    <section className="relative overflow-hidden bg-[#0A0A0A] border-b-2 border-white dark:border-[#262626] min-h-[320px] flex items-center">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-10">
        ...
        <div className="max-w-3xl flex-1 pt-4">
          {/* Pagination / Subtitle */}
          {breadcrumbs ? (
            <nav className="flex items-center flex-wrap gap-x-2 gap-y-1 text-[10px] sm:text-xs font-mono tracking-[0.2em] text-white/60 mb-4 uppercase" aria-label="Breadcrumb">
              {breadcrumbs.map((crumb, index) => (
                <React.Fragment key={crumb.label + index}>
                  {crumb.href ? (
                    ...
                    <span className="text-white font-bold truncate max-w-[220px] sm:max-w-xs md:max-w-md inline-block align-bottom">{crumb.label}</span>
                  )}
